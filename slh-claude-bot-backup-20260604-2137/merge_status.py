...
# --- 3) וודא ש‑/backup המערכתי קיים (אם לא, הוסף) ---
if "@dp.message(Command(\"backup\"))" not in code:
    new_backup = r'''
...
'''
    # הוסף אחרי sales
    code = re.sub(r"(@dp\.message\(Command\(\"sales\"\)\).*?\n\s*await msg\.answer\(.*?\))", r"\1" + new_backup, code, count=1, flags=re.DOTALL)

# --- 4) וודא ש‑/deploy קיים ---
...
'''
    code = re.sub(r"(@dp\.message\(Command\(\"sales\"\).*?\n\s*await msg\.answer\(.*?\))", r"\1" + new_deploy, code, count=1, flags=re.DOTALL)

with open("bot.py", "w", encoding="utf-8") as f:
    f.write(code)

print("✅ /status, /backup, /deploy אוחדו בהצלחה")
