...
from app.services.withdrawals_query import approve_withdrawal, mark_withdraw_sent
...
async def main():
    ...
    try:
        ...
        wallet = "E2E_" + uuid.uuid4().hex[:12].upper()
        tx_hash = "E2E_TX_" + uuid.uuid4().hex[:16].upper()
        ...
        sent_result = await mark_withdraw_sent(withdrawal_id, ADMIN_USER_ID, tx_hash)
        ...
        async with db.pool.acquire() as conn:
            ...
            withdrawal_row = await row(conn, """
                SELECT id, user_id, amount, wallet, status, tx_hash, created_at, approved_at, processed_at, reviewed_by
                FROM withdrawals
                WHERE id = $1
            """, withdrawal_id)
