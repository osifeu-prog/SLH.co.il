...
SELECT
  NULL,
  'schema.finance_functions.v1',
  '{"ok":true,"functions":["finance_post_user_reward","finance_reject_withdrawal_release","finance_mark_withdrawal_sent_consume"]}',
  NOW()
WHERE NOT EXISTS (
  SELECT 1 FROM audit_log WHERE event_type='schema.finance_functions.v1'
);
