...
CREATE OR REPLACE FUNCTION finance_approve_withdrawal_reserve(
...
BEGIN
  ...
  WHERE NOT EXISTS (
    SELECT 1
    FROM audit_log
    WHERE event_type = 'withdraw.approved'
...
SELECT
  NULL,
  'schema.finance_approve_withdrawal_reserve.v1',
  '{"ok":true}',
  NOW()
WHERE NOT EXISTS (
  SELECT 1 FROM audit_log WHERE event_type='schema.finance_approve_withdrawal_reserve.v1'
);
