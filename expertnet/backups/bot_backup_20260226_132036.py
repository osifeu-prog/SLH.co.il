...
def get_main_menu(uid):
    keyboard = [
        [InlineKeyboardButton(" ??? Mini-App", web_app=WebAppInfo(url="https://expertnet-app.vercel.app"))],
        [InlineKeyboardButton(" ??????", callback_data='me'), InlineKeyboardButton(" ??? ??", callback_data='market')],
        [InlineKeyboardButton(" ????? ????", callback_data='reg_wallet'), InlineKeyboardButton(" Vault", callback_data='vault_info')]
    ]
    if str(uid) == ADMIN_ID:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'me':
        ...
        bal = w3.from_wei(w3.eth.get_balance(wallet), 'ether') if wallet != "?? ?????" else 0
        msg = f"<b> ?????? ?????:</b>\n ????: <code>{wallet}</code>\n ????: <b>{bal:.4f} BNB</b>"
        ...
        kb = [[InlineKeyboardButton(" ???? ???? Vault", callback_data='admin_set_key')],
              [InlineKeyboardButton(" ????? ?????", callback_data='admin_broadcast')],
        ...
        await query.edit_message_text("????? ????:", reply_markup=get_main_menu(uid))
