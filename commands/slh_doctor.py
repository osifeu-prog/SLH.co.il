...
async def check_railway() -> dict:
    if not RAILWAY_TOKEN or not PROJECT_ID:
        return {
            ...
            "detail": "RAILWAY_API_TOKEN or RAILWAY_PROJECT_ID missing",
        }
...
async def check_ai_key() -> dict:
    for env_var, prefix in [
        ...
        if val:
            ...
            return {
                "ok": valid,
                "provider": env_var.split("_")[0].title(),
                "detail": "key present" if valid else "key format unexpected",
            }
...
def build_report(railway, db, redis, ai) -> str:
    ...
    if r_service:
        lines.append(f"   service: {r_service} | deploy: {r_id}")
    if not railway["ok"] and "detail" in railway:
        lines.append(f"   ⚠️ {railway['detail']}")

    lines.append("")

    db_icon = _icon(db)
    if db["ok"]:
        lines.append(f"{db_icon} *Database* — {db.get('latency_ms', '?')}ms")
    else:
        lines.append(f"{db_icon} *Database* — {db.get('detail', 'error')}")

    r2_icon = _icon(redis)
    if redis.get("ok") is None:
        lines.append(f"{r2_icon} *Redis* — {redis.get('detail', 'skip')}")
    elif redis["ok"]:
        lines.append(f"{r2_icon} *Redis* — {redis.get('latency_ms', '?')}ms")
    else:
        lines.append(f"{r2_icon} *Redis* — {redis.get('detail', 'error')}")
    ...
    if all(c is True for c in checks):
        ...
        lines.append("🔴 *Issues detected — check logs*")
    else:
        lines.append("🟡 *Partial — some checks skipped*")
