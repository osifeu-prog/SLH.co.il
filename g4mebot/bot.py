...
async def show_candidate(event):
    ...
    text = (
        f"?? *{c['display_name']}* ן¿½ {c['age']}\n"
        f"?? {c.get('city') or '-'} ן¿½ {'? ?????' if c.get('verified') else ''}\n\n"
        f"{c.get('bio') or '(??? ???)'}\n\n"
...
async def show_matches(event):
    ...
    lines = ["?? *??????? ???:*\n"]
    for m in matches[:10]:
        tg = f" ן¿½ t.me/{m['tg_username']}" if m.get("tg_username") else ""
        verified = " ?" if m.get("verified") else ""
        lines.append(f"ן¿½ {m['display_name']} ן¿½ {m['age']}{verified}{tg}")
...
async def cmd_profile(m: Message):
    ...
    if not profile.get("exists"):
        ...
        return
    text = (
        f"?? *{profile['display_name']}* ן¿½ {profile['age']}\n"
        f"?? {profile.get('city','-')}\n"
...
async def cmd_site(m: Message):
    await m.answer(
        "?? *???? ???? ???? SLH*\n\n"
        f"ן¿½ [Dashboard]({web_url('/dashboard.html', m.from_user.id)})\n"
        f"ן¿½ [?????? Dating]({web_url('/dating.html', m.from_user.id)})\n"
        f"ן¿½ [Wallet]({web_url('/wallet.html', m.from_user.id)})\n"
        f"ן¿½ [Community]({web_url('/community.html', m.from_user.id)})\n"
        f"ן¿½ [Sudoku]({web_url('/sudoku.html', m.from_user.id)})\n",
        parse_mode="Markdown",
