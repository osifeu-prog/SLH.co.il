...
def main_menu_kb(is_instructor: bool = False, is_approved: bool = False) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton(text="?????�", callback_data="menu:support")])
...
async def cmd_buy(msg: Message) -> None:
    ...
    try:
        ...
        try:
            await msg.answer(
                "שגיאה בטעינת הקורסים. נסה שוב או פנה ל?????� "
                f"{SUPPORT_HANDLE}.",
...
async def cmd_my_licenses(msg: Message) -> None:
    ...
    try:
        ...
        try:
            await msg.answer(
                "שגיאה בטעינת הרישיונות. נסה שוב או פנה ל?????� "
                f"{SUPPORT_HANDLE}.",
                reply_markup=main_menu_kb(),
            )
        except Exception:
...
async def cb_course(cq: CallbackQuery) -> None:
    try:
        ...
        if c["materials_url"]:
            ...
            included_lines.append("• תוכן בהכנה — פנה ל?????� לפרטים")
        ...
        text = (
            f"<b>{c['title_he']}</b>\n"
            f"{instructor_line}\n"
            f"{c['description_he'] or 'תיאור מלא בקרוב — פנה ל?????� למידע.'}\n\n"
            f"{included}\n\n"
...
async def cb_pay(cq: CallbackQuery) -> None:
    ...
    try:
        ...
        if not payment_id:
            await cq.message.edit_text(
                "שגיאה ביצירת התשלום. נסה שוב מאוחר יותר או פנה ל?????� "
                f"{SUPPORT_HANDLE}.",
                reply_markup=main_menu_kb(),
            )
            return
...
async def _wait_and_grant(
    ...
    try:
        ...
        try:
            await bot.send_message(
                chat_id,
                f"שגיאה באימות. פנה ל?????� {SUPPORT_HANDLE} "
                f"(מזהה: {payment_id}).",
            )
        except Exception:
