...
async def digest_send(
    ...
    lines.append(
        f"???? secrets: <b>{stats['total']}</b> ן¿½ "
        f"??????: <b>{stats['broken']}</b> ן¿½ "
        f"??????: <b>{stats['overdue']}</b>"
    )
    ...
    if recent_alerts:
        ...
        for r in recent_alerts:
            lines.append(f"  ן¿½ <code>{r['alert_type']}</code>: {r['n']}")
    ...
    if broken_rows:
        ...
        for r in broken_rows:
            lines.append(
                f"  ן¿½ {r['display_name']} (<code>{r['key_name']}</code>) - {r['last_health_result']}"
            )
    ...
    if overdue_rows:
        ...
        for r in overdue_rows:
            ...
            lines.append(
                f"  ן¿½ {r['display_name']} - {age}d (cadence {r['rotation_cadence_days']}d)"
            )
