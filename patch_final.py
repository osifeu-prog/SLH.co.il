...
async def ai_chat(message: types.Message):
    ...
    try:
        ...
        answer = f"⚠️ Groq error: {str(e)[:100]}"
    # 2. Gemini fallback
    if not answer or answer.startswith("⚠️"):
        try:
            ...
            pass
    if not answer:
        answer = "❌ All AI engines unavailable."
...
# --- /wallet (שיפור) ---
c = c.replace(
    'async def cmd_wallet(msg): await msg.answer("💰 Wallet (coming soon)")',
...
print("✅ AI + wallet + simdeposit upgraded.")
