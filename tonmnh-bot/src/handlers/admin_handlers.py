...
def register_admin_handlers(bot, core_services):
    ...
    def admin_main(message):
        ...
        markup.add(InlineKeyboardButton(" ??? ??????", callback_data="adm_revenue"))
        markup.add(InlineKeyboardButton(" ????? ??????", callback_data="adm_merchants"))
        markup.add(InlineKeyboardButton(" ??? ?????? (Last 5)", callback_data="adm_logs"))
