...
def main_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="?? Oracle", callback_data="cmd_oracle"),
         InlineKeyboardButton(text="?? Peace Game", callback_data="cmd_peace")],
        ...
        [InlineKeyboardButton(text="?? ?? ???????", url="https://slh-nft.com/investor-landing/")],
    ])
...
async def cmd_help(msg: Message):
    await msg.answer("?? **??????**: /start /help /crypto /guide /donate /tasks /menu /pay /buy /upgrade /oracle /peace /seed /sysinfo /profile /myid /leaderboard /checkin /points /daily /backup /broadcast /wallet /deposit /tap /referral /admin /users /morning /doctor /statusapi /test /crm /events /support /roadmap /transfer /faq /tutorial /progress /done /commission /links /about /healthcheck /identity /myidentity /community /wallet /game /invest")
...
async def cmd_crypto(msg: Message):
    try:
        ...
        await msg.answer("?? Crypto prices temporarily unavailable.")
...
async def cmd_pay(msg: Message):
    await msg.answer_invoice(title="SLH Premium",description="???? ?????? ????????",payload="premium_monthly",currency="XTR",prices=[LabeledPrice(label="SLH Premium", amount=5000)],start_parameter="premium",provider_token="")
...
async def cmd_checkin(msg: Message):
    ...
    if user["last_checkin"]==today: await msg.answer("?? Already checked in today!"); return
...
async def cmd_healthcheck(msg: Message):
    ...
    for f in [CONTACTS_FILE, POINTS_FILE]:
        results.append(f"? {f}: Exists" if os.path.exists(f) else f"?? {f}: Not yet created")
    try:
