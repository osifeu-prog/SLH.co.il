# -*- coding: utf-8 -*-
"""
SLH AI Chat ג€” Multi-provider backend with automatic fallback
Providers (in priority order):
  1. Groq (free tier ג€” llama-3.3-70b)
  2. Google Gemini (free tier ג€” gemini-2.0-flash)
  3. Together.ai (free tier ג€” meta-llama/Llama-3.3-70B-Instruct-Turbo-Free)
  4. OpenAI (paid ג€” gpt-4o, if key available)
...
SLH_SYSTEM_PROMPT = """You are SLH AI Assistant ג€” the smart assistant for the SLH Ecosystem crypto investment platform.

About SLH:
- SLH is an Israeli crypto ecosystem with 20+ Telegram bots, a website (slh-nft.com), and its own token
- SLH Token: BSC contract 0xACb0A09414CEA1C879c67bB7A877E4e19480f022, price ג‚×444 ($121.64)
...
async def _call_groq(client: httpx.AsyncClient, messages: list) -> tuple[str, str]:
    """Groq ג€” free tier, Llama 3.3 70B, very fast."""
...
async def _call_gemini(client: httpx.AsyncClient, messages: list) -> tuple[str, str]:
    """Google Gemini ג€” free tier, Gemini 2.0 Flash."""
...
async def _call_together(client: httpx.AsyncClient, messages: list) -> tuple[str, str]:
    """Together.ai ג€” free tier, Llama 3.3 70B Turbo."""
...
async def ai_chat(req: ChatRequest):
    ...
    if not providers:
        raise HTTPException(status_code=503, detail="AI service not configured ג€” no API keys set")
...
async def _check_and_reserve_aic(user_id: int, expected_cost: float) -> tuple[float, float]:
    ...
    if _aic_pool_ref is None:
        return (float("inf"), 0)  # AIC not wired ג€” pass through
...
async def ai_chat_metered(req: MeteredChatRequest):
    ...
    # Estimate cost ג€” assume free tier first
        ...
        for name, call_fn in providers:
            try:
                ...
                if tier == "paid" and balance_before < actual_cost:
                    # downgrade ג€” should not happen because we already reserved free cost,
