# -*- coding: utf-8 -*-
"""
Creator Economy ג€” XP = ROI metric + SLH Index.
...
Endpoints:
  GET  /api/creator/xp/{user_id}         ג€” user's XP (ROI ratio)
  GET  /api/creator/slh-index            ג€” global median XP + sparkline data
  GET  /api/creator/shop/{user_id}       ג€” personal shop view with stats
  POST /api/creator/roi/snapshot         ג€” admin; recompute snapshot for a user
  POST /api/creator/roi/snapshot/all     ג€” admin; recompute for all users (cron)

XP formula:
  user_xp = total_portfolio_value_now / total_fiat_invested
  where:
    total_portfolio_value_now = sum(SLH_held ן¿½- current_SLH_price
                                   + AIC_held ן¿½- current_AIC_price
...
A user with 0 fiat invested is "xp = גˆ (pure gain)" ג€” not counted in index.
"""
...
# Current prices ג€” editable via env vars. TODO: fetch live from oracle.
SLH_PRICE_USD = float(os.getenv("SLH_PRICE_USD", "121.6"))  # ~ג‚×444 / 3.65
AIC_PRICE_USD = float(os.getenv("AIC_PRICE_USD", "0.001"))
REP_VALUE_USD = float(os.getenv("REP_VALUE_USD", "0.01"))  # REP is symbolic ג€” 0.01$ per point
...
async def _compute_xp(conn, user_id: int) -> dict:
    ...
    # SLH held ג€” try multiple possible tables
    slh_held = 0.0
    for q in (
    ...
    return {
        ...
        "xp_ratio": xp_ratio,
        "xp_display": "גˆ" if xp_ratio is None else f"{xp_ratio:.2f}",
        "status": (
...
async def slh_index():
    """Global SLH Index ג€” median XP across users with >0 fiat invested."""
    if _pool is None:
        raise HTTPException(500, "db pool not initialized")
    async with _pool.acquire() as conn:
        await _ensure_tables(conn)
        ...
        # Trend ג€” compare to 24h ago via heuristic (simplified)
        ...
        return {
            ...
            "trend": trend,
            "peak_xp_label": "גˆ" if any(r.get("xp_ratio") is None for r in rows) else round(xps[-1], 2),
...
async def personal_shop(user_id: int):
    """Personal creator shop ג€” listings + earnings + XP."""
    if _pool is None:
        raise HTTPException(500, "db pool not initialized")
    async with _pool.acquire() as conn:
        await _ensure_tables(conn)
...
async def purchase_complete(req: PurchaseCompleteReq):
    ...
    return {
        ...
        "message": f"׳¨׳›׳™׳©׳” ׳”׳•׳©׳׳׳” ֲ· ׳”׳׳•׳›׳¨ #{seller_id} ׳§׳™׳‘׳ ${usd:.2f}, XP ׳¢׳•׳“׳›׳.",
    }
...
async def refresh_all_snapshots(x_admin_key: Optional[str] = Header(None)):
    """Admin: recompute XP for all users ג€” run as cron."""
    admin_keys = [k.strip() for k in os.getenv("ADMIN_API_KEYS", "slh2026admin").split(",") if k.strip()]
    if x_admin_key not in admin_keys:
        raise HTTPException(403, "admin key required")
    if _pool is None:
