...
Endpoints:
  POST /api/payment/ton/auto-verify     ג€” verify TON tx, grant premium
  POST /api/payment/bsc/auto-verify     ג€” verify BSC/BNB tx, grant premium
  GET  /api/payment/status/{user_id}    ג€” check user premium + last deposit
  GET  /api/payment/config              ג€” public config (addresses, min amounts)
  POST /api/payment/external/record     ג€” record a 3rd-party fiat payment (Stripe/PayBox/Bit)
  POST /api/payment/receipt              ג€” issue a SLH digital receipt
  GET  /api/payment/geography/summary   ג€” dashboard: payments by country/currency
...
PREMIUM_MIN_ILS = float(os.getenv("PREMIUM_MIN_ILS", "1"))       # for testing ג€” 1 ILS = symbolic

# Testnet mode ג€” set PAYMENT_MODE=testnet in Railway to switch to TON Testnet + BSC Chapel.
# Default is "mainnet" ג€” real money. Testnet = fake money, safe for QA/tutorials.
PAYMENT_MODE = os.getenv("PAYMENT_MODE", "mainnet").lower().strip()
IS_TESTNET = PAYMENT_MODE == "testnet"

if IS_TESTNET:
    # BSC Testnet (Chapel) ג€” public RPCs, free tBNB from https://testnet.binance.org/faucet-smart
    BSC_GENESIS_ADDRESS = os.getenv("BSC_TESTNET_ADDRESS", BSC_GENESIS_ADDRESS).lower()
    # TON Testnet ג€” https://testnet.toncenter.com
    # (TON_PAY_ADDRESS on testnet is a separate wallet ג€” user sets via env)
...
async def bsc_auto_verify(req: BscVerifyReq, request: Request):
    ...
    if IS_TESTNET:
        # BSC Testnet (Chapel) ג€” free tBNB at https://testnet.binance.org/faucet-smart
        bsc_rpcs = [
        ...
        # Mainnet ג€” Binance dataseed + fallbacks
        bsc_rpcs = [
...
# Record-only ג€” the actual card processing happens on the provider side,
...
async def external_payment_record(req: ExternalPaymentRecord, request: Request):
    ...
    return {
        ...
        "receipt": rcpt,
        "message": f"׳×׳©׳׳•׳ {provider} ׳ ׳¨׳©׳" + (" ׳•-Premium ׳”׳•׳₪׳¢׳" if req.grant_premium else ""),
    }
...
async def payment_config():
    return {
        ...
        "gas_estimate": {
            ...
            "bsc_network_fee_bnb": 0.0003,  # ~21000 gas ן¿½- 5 gwei
            "bsc_usd_equivalent": 0.20,  # rough
        },
        "test_tx_note": (
            "׳׳‘׳“׳™׳§׳× flow ׳¨׳-׳©׳•׳ ׳” ׳׳•׳׳׳¥: 0.01 TON (׳›׳•׳׳ ׳¢׳׳׳” ~0.005 TON) "
            "׳-׳• 0.001 BNB (~$0.63). ׳-ן¿½-׳¨׳™ ׳©׳”test ׳¢׳•׳‘׳“, ׳₪׳¨׳׳™׳™׳“ ׳¨׳’׳™׳ ׳-0.5 TON / 0.05 BNB."
        ),
    }
