"""
SLH AIC Â· AI Credits â€” 6th internal token
==========================================
Bridges user activity â†’ AI API costs â†’ system value.
See ops/AIC_TOKEN_DESIGN.md for full design.

Phase 1 MVP endpoints (read + earn/spend flows):
  GET  /api/aic/balance/{user_id}     â€” current balance + lifetime stats
  GET  /api/aic/transactions/{user_id} â€” tx history
  POST /api/aic/earn                  â€” add AIC (auto-triggered by system events)
  POST /api/aic/spend                 â€” deduct AIC for AI call
  GET  /api/aic/stats                 â€” global: supply, daily flow, top holders
  POST /api/admin/aic/mint            â€” admin: create AIC against reserve
  GET  /api/admin/aic/reserve         â€” reserve status
...
# â•â•â•â•â•â•â• Public endpoints â•â•â•â•â•â•â•
...
# â•â•â•â•â•â•â• Admin endpoints â•â•â•â•â•â•â•
