# -*- coding: utf-8 -*-
"""
Agent Hub ג€” multi-agent coordination API for SLH Spark.

Backs the /agent-hub.html "ICQ of AI agents" page. Every paste from Claude,
Copilot, Gemini, Grok, or Osif lands here ג€” shared across devices, searchable,
priority-tagged. The website keeps localStorage as an offline fallback only.

Endpoints (prefix /api/agent-hub):
  POST   /message          ג€” insert a new coordination message
  GET    /messages         ג€” paginated feed with filters (source, priority, topic, search)
  GET    /stats            ג€” aggregate counts by source + priority
  DELETE /message/{id}     ג€” admin-only (X-Admin-Key header)
  POST   /bulk-import      ג€” admin-only, useful for migrating localStorage dumps
...
creator_economy.py and arkham_bridge.py ג€” comma-separated ADMIN_API_KEYS env
...
async def init_agent_hub_tables():
    ...
    if _pool is None:
        logger.warning("[agent-hub] init skipped ג€” pool not set")
        return
    try:
...
async def list_messages(
    ...
    try:
        ...
        # Table may not exist yet ג€” return empty gracefully
        ...
        async with _pool.acquire() as conn:
            ...
            for r in by_source_rows:
                ...
                if src in by_source:
                    ...
                    # Unknown / legacy label ג€” expose as-is so Osif can see it
                    by_source[src] = int(r["n"])
