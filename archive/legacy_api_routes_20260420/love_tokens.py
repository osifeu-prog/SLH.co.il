# -*- coding: utf-8 -*-
"""
Love Tokens ג€” virtual affection economy.

STATUS: STUB (schema + endpoints live, but not wired to any UI yet).
Disabled on the frontend ג€” users see nothing. DB tables created lazily.

Three token types:
  HUG (ן¿½-)     ג€” 5 AIC to send ג€” gentle, friend-level
  KISS (נ’‹)    ג€” 20 AIC        ג€” intimate, match-level
  HANDSHAKE (נ₪) ג€” 2 AIC       ג€” formal, greeting-level
...
Endpoints:
  GET  /api/love/balance/{user_id}    ג€” all 3 token balances (no auth ג€” public profile view)
  GET  /api/love/received/{user_id}   ג€” list of last N received tokens + from whom (last 100)
  POST /api/love/send                  ג€” send token (requires AIC, respects daily cap)
  GET  /api/love/config                ג€” current pricing + daily caps (read-only)

Tables (auto-created on first call):
  love_token_balances  ג€” per-user per-type counter
  love_token_transfers ג€” append-only ledger
...
# Editable pricing + caps ג€” no UI yet. Change via Railway env vars when ready.
LOVE_PRICES = {
...
async def love_config():
    return {
        ...
        "icons": {"hug": "ן¿½-", "kiss": "נ’‹", "handshake": "נ₪"},
        "note": "Stub ג€” UI not yet wired. Enable via env LOVE_TOKENS_ENABLED=1 after UI ships.",
    }
...
async def love_send(req: SendLoveReq):
    ...
    async with _pool.acquire() as conn:
        ...
        # Charge AIC ג€” uses aic_tokens module if available, else skip (stub mode)
        try:
            await conn.execute(
            ...
            pass  # AIC not wired yet ג€” running free until enabled
