# -*- coding: utf-8 -*-
"""
Blockchain Verification Helpers ג€” REAL public-API clients
==========================================================
...
caller ג€” on failure they return a dict with ``source`` set to a marker like
...
SLH_DECIMALS = 15  # Non-standard ג€” SLH uses 15, not the ERC-20 default 18
...
async def _http_get_json(
    ...
    Never raises ג€” callers can always safely inspect the returned dict.
    """
    if aiohttp is None:
        logger.error("aiohttp not available ג€” cannot reach %s", url)
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url, params=params, headers=merged_headers) as resp:
                ...
                except Exception as e:  # noqa: BLE001 ג€” JSON parse failure
...
class GoPlusClient:
    ...
    def _truthy(val: Any) -> bool:
        """GoPlus returns '1'/'0' strings, bools, ints ג€” normalize."""
        if val is None:
            return False
...
class TonCenterClient:
    """Free TON wallet info ג€” used for TON address verification."""
...
class BlockchainThreatClient:
    """
    Thin faֳ§ade that combines GoPlus + BscScan + Chainabuse into the shape the
