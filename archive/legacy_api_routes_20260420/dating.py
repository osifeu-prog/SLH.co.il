# -*- coding: utf-8 -*-
"""
SLH Dating ג€” Quality-gated matchmaking
========================================
...
Endpoints:
  POST /api/dating/profile            ג€” create or update profile
  GET  /api/dating/profile/{user_id}  ג€” get my profile (private)
  GET  /api/dating/profile/{user_id}/public ג€” public view of another user
  POST /api/dating/match/candidates   ג€” return 10 potential matches
  POST /api/dating/match/action       ג€” like/pass/superlike
  GET  /api/dating/matches/{user_id}  ג€” mutual matches (can chat)
  GET  /api/dating/stats              ג€” global (not personal)
...
async def create_or_update_profile(req: ProfileReq):
    ...
    return {"ok": True, "user_id": req.user_id, "message": "׳₪׳¨׳•׳₪׳™׳ ׳ ׳©׳׳¨. ׳ ׳¡׳” ׳׳׳¦׳•׳- ׳”׳×׳-׳׳•׳×."}
...
async def get_public_profile(user_id: int):
    """Public view ג€” no phone/email/contact details."""
    if _pool is None:
        raise HTTPException(500, "db pool not initialized")
    async with _pool.acquire() as conn:
        await _ensure_dating_tables(conn)
...
async def match_action(req: ActionReq):
    ...
    return {
        ...
        "action": req.action,
        "is_match": is_match,
        "message": "נ‰ ׳”׳×׳-׳׳” ׳”׳“׳“׳™׳×! ׳-׳×׳ ׳™׳›׳•׳׳™׳ ׳׳“׳‘׳¨ ׳¢׳›׳©׳™׳•." if is_match else "׳”׳₪׳¢׳•׳׳” ׳ ׳©׳׳¨׳”.",
    }


@router.get("/matches/{user_id}")
async def my_matches(user_id: int):
    """All mutual matches ג€” people who liked you back."""
    if _pool is None:
        raise HTTPException(500, "db pool not initialized")
    async with _pool.acquire() as conn:
        await _ensure_dating_tables(conn)
