...
async def cmd_start(msg: Message):
    ...
    await msg.answer(msg_welcome(name), parse_mode=None, reply_markup=markup)
...
async def cmd_register(msg: Message):
    ...
    try:
        ...
        conn.commit()
        await msg.answer(msg_register_success(uname), parse_mode=None)
...
async def cmd_donate(msg: Message):
    await msg.answer(msg_donate(), parse_mode=None, reply_markup=kb_donate())
...
async def cmd_checkin(msg: Message):
    ...
    if row and row[2] == today:
        await msg.answer(msg_checkin_already(), parse_mode=None)
        conn.close()
        return
    ...
    conn.commit()
    conn.close()
    await msg.answer(msg_checkin_success(name, bonus, new_points, streak), parse_mode=None, reply_markup=kb_after_checkin())
...
async def cmd_points(msg: Message):
    ...
    conn.close()
    if row:
        await msg.answer(msg_points(msg.from_user.full_name, row[0], "free"), parse_mode=None, reply_markup=kb_after_points())
    else:
        await msg.answer("×œ× ×¨×©×•×. ×”×©×ª×ž×© ×‘-/register")
...
async def cmd_leaderboard(msg: Message):
    ...
    conn.close()
    if not rows:
        await msg.answer("××™×Ÿ × ×ª×•× ×™×")
        return
    entries = [{"username": r[0], "points": r[1]} for r in rows]
    await msg.answer(msg_leaderboard(entries), parse_mode=None, reply_markup=kb_leaderboard())
...
async def cmd_status(msg: Message):
    ...
    conn.close()
    await msg.answer(msg_status(raised, 50000, total, 30), parse_mode=None, reply_markup=kb_status())
...
async def cmd_users(msg: Message):
    ...
    rows = cur.fetchall()
    conn.close()
    if not rows: return await msg.answer("××™×Ÿ ×ž×©×ª×ž×©×™×")
...
async def cmd_broadcast(msg: Message):
    ...
    if len(parts) < 2: return await msg.answer("×©×™×ž×•×©: /broadcast <×”×•×“×¢×”>")
    conn = get_db()
    cur = conn.cursor()
    ...
    await msg.answer(f"ðŸ“¢ × ×©×œ×— ×œ-{sent}/{len(users)} ×ž×©×ª×ž×©×™×.")
...
async def cmd_morning(msg: Message):
    ...
    top3 = cur.fetchall()
    conn.close()
    lb = "\n".join(f"{i+1}. {r[0]} - {r[1]} × ×§×•×“×•×ª" for i, r in enumerate(top3))
    await msg.answer(f"ðŸŒ… ×“×•×— ×‘×•×§×¨\n×ª××¨×™×š: {today}\n×¡×”\"×› ×ž×©×ª×ž×©×™×: {total} (+{new_today})\n×¦'×§-××™×Ÿ ×”×™×•×: {checked}\n×˜×•×¤ 3:\n{lb}")
...
async def cmd_stats(msg: Message):
    ...
    conn.close()
    await msg.answer(f"ðŸ“Š ×¡×˜×˜×™×¡×˜×™×§×•×ª ×§×ž×¤×™×™×Ÿ\n×ª×•×ž×›×™×: {total}\n×¦'×§-××™×Ÿ ×”×™×•×: {checked}\n×¡×”\"×› × ×§×•×“×•×ª: {total_points}\nhttps://slh-nft.com/campaign/")

@dp.message(Command("help"))
async def cmd_help(msg: Message):
    await msg.answer(msg_help(), parse_mode=None, reply_markup=kb_help())
...
async def cmd_referral(msg: Message):
    ...
    conn.close()
    await msg.answer(msg_referral(msg.from_user.full_name, link, count), parse_mode=None, reply_markup=kb_referral())

@dp.message(Command("roadmap"))
async def cmd_roadmap(msg: Message):
    await msg.answer(msg_roadmap(), parse_mode=None, reply_markup=kb_roadmap())

@dp.message(Command("feedback"))
async def cmd_feedback(msg: Message):
    if not msg.text.split(" ", 1)[1:]:
        return await msg.answer(msg_error_no_args("feedback"), parse_mode=None, reply_markup=kb_back_to_menu())
    ...
    conn.commit()
    conn.close()
    await msg.answer(msg_feedback_success(), parse_mode=None, reply_markup=kb_back_to_menu())

@dp.message(Command("daily"))
async def cmd_daily(msg: Message):
    missions = [{"title": "Check-in", "points": 5, "done": False}, {"title": "Referral", "points": 10, "done": False}]
    await msg.answer(msg_daily(missions), parse_mode=None, reply_markup=kb_daily())

@dp.message(Command("myid"))
async def cmd_myid(msg: Message):
    await msg.answer(f"ðŸªª ×”-ID ×©×œ×š: {msg.from_user.id}")

@dp.message(Command("backup"))
async def cmd_backup(msg: Message):
    await msg.answer("×’×™×‘×•×™ ×©×ž×•×¨ (stub)")

@dp.message(Command("tasks"))
async def cmd_tasks(msg: Message):
    await msg.answer("×ž×©×™×ž×•×ª: /checkin, /referral, /feedback")

@dp.message(Command("support"))
async def cmd_support(msg: Message):
    await msg.answer("×”×¦×˜×¨×¤×• ×œ×§×”×™×œ×”: https://t.me/SLH_Claude_bot")
...
async def cmd_commission(msg: Message):
    if msg.from_user.id not in ADMIN_IDS: return await msg.answer(msg_error_admin_only())
    await msg.answer("×¢×ž×œ×”: 5% ×œ×¢×¡×§×”. ×©×™× ×•×™: /set_commission <rate>")
...
async def cmd_wallet(msg: Message):
    try:
        ...
        await msg.answer(f"ðŸ’° ×™×ª×¨×ª ×”××¨× ×§: ${bal:.2f}")
    except Exception as e:
...
async def cmd_transfer(msg: Message):
    ...
    if len(parts) != 3: return await msg.answer("×©×™×ž×•×©: /transfer <×ž×–×”×”> <×¡×›×•×>")
    try:
        to = int(parts[1])
        amt = float(parts[2])
        if transfer(msg.from_user.id, to, amt):
            ...
            await msg.answer(f"âœ… ×”×•×¢×‘×¨ ${amt:.2f} ×œ-{to}")
        else:
            await msg.answer("âŒ ×”×¢×‘×¨×” × ×›×©×œ×”  ×™×ª×¨×” ×œ× ×ž×¡×¤×§×ª")
    except Exception as e:
        await msg.answer(f"×©×’×™××”: {e}")
...
async def cmd_deposit(msg: Message):
    ...
    await msg.answer(f"ðŸ’Ž ×©×œ×— TON ×œ:\n{ton}\n×•××– ×¤× ×” ×œ××“×ž×™×Ÿ")

@dp.message(Command("store"))
async def cmd_store(msg: Message):
    parts = msg.text.split(" ", 2)
    if len(parts) < 2: return await msg.answer("×©×™×ž×•×©: /store create <×©×> <×ª×™××•×¨> ××• /store view <id>")
    if parts[1] == "create":
        if len(parts) < 3: return await msg.answer("×©×™×ž×•×©: /store create <×©×> <×ª×™××•×¨>")
        sid = create_store(msg.from_user.id, parts[2], parts[3] if len(parts) > 3 else "")
        await msg.answer(f"ðŸª ×—× ×•×ª × ×•×¦×¨×”! ID: {sid}")
    elif parts[1] == "view":
        try:
            store = get_store(int(parts[2]))
            await msg.answer(f"×—× ×•×ª: {store['name']}\n{store['description']}")
        except: await msg.answer("×©×™×ž×•×©: /store view <id>")
    else: await msg.answer("×¤×§×•×“×” ×œ× ×ž×•×›×¨×ª")
...
async def cmd_add_product(msg: Message):
    ...
    try:
        ...
        await msg.answer(f"ðŸ“¦ ×ž×•×¦×¨ × ×•×¡×£! ID: {pid}")
    except Exception as e: await msg.answer(f"×©×’×™××”: {e}")

@dp.message(Command("products"))
async def cmd_products(msg: Message):
    parts = msg.text.split()
    if len(parts) < 2: return await msg.answer("×©×™×ž×•×©: /products <store_id>")
    prods = list_products(int(parts[1]))
    if not prods: return await msg.answer("××™×Ÿ ×ž×•×¦×¨×™×")
...
async def cmd_buy(msg: Message):
    parts = msg.text.split()
    if len(parts) < 2: return await msg.answer("×©×™×ž×•×©: /buy <product_id>")
    pid = int(parts[1])
    conn = get_db()
    cur = conn.cursor()
    ...
    row = cur.fetchone()
    conn.close()
    if not row: return await msg.answer("×ž×•×¦×¨ ×œ× × ×ž×¦×")
    price, seller = row
    if create_order(msg.from_user.id, pid, price, seller):
        ...
        await msg.answer(f"âœ… ×¨×›×™×©×” ×‘×•×¦×¢×”! {price}$ ×©×•×œ×ž×•")
    else:
        await msg.answer("âŒ ×¨×›×™×©×” × ×›×©×œ×”  ×™×ª×¨×” ×œ× ×ž×¡×¤×§×ª")
...
async def cmd_profile(msg: Message):
    ...
    conn.close()
    if user:
        await msg.answer(f"ðŸ‘¤ ×ž×©×ª×ž×©: {user[0]}\nðŸŽ– Tier: {user[1]}\nðŸ’° ×™×ª×¨×”: ${bal:.2f}\nðŸ“… ×”×¦×˜×¨×£: {user[2]}")
    else:
        await msg.answer("×œ× ×¨×©×•×. ×”×©×ª×ž×© ×‘-/register")
...
async def cmd_leaders(msg: Message):
    ...
    conn.close()
    if rows:
        await msg.answer("ðŸ† ×˜×•×¤ 10:\n" + "\n".join(f"{i+1}. {r[0]} - {r[1]} × ×§×•×“×•×ª" for i, r in enumerate(rows)))
    else:
        await msg.answer("××™×Ÿ × ×ª×•× ×™×")
...
async def cmd_crm(msg: Message):
    ...
    total_points = cur.fetchone()[0] or 0
    conn.close()
    await msg.answer(f"ðŸ“Š CRM Report\nTotal users: {total}\n\nTiers:\n{tier_lines}\n\nTotal points: {total_points}")
...
async def cmd_admin(msg: Message):
    ...
    products = cur.fetchone()[0]
    conn.close()
    text = (f"ðŸ‘‘ *SLH Admin Panel*\nâ”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n\n"
            f"ðŸ‘¥ Users: {total} total, {checked} checked in today\n"
            f"ðŸ“Š Activity: {events_today} events today\n"
            f"ðŸ’° Revenue (24h): ${revenue:.2f}\n"
            f"ðŸ’³ Active wallets: {active_wallets}\n"
            f"ðŸª Marketplace: {stores} stores, {products} products\n\n"
            f"ðŸ“Œ Quick actions:\n/users\n/broadcast\n/doctor\n/dashboard")
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="ðŸ“Š Dashboard", callback_data="cmd_dashboard"),
         InlineKeyboardButton(text="ðŸ©º Doctor", callback_data="cmd_doctor")]
    ])
    await msg.answer(text, parse_mode=None, reply_markup=keyboard)
...
async def cmd_statusapi(msg: Message):
    ...
    try:
        ...
        db_ok = "âŒ"
    conn.close()
    redis_ok = "âœ…" if os.getenv("REDIS_URL") else "âšª"
    ai_ok = "âœ…" if os.getenv("GROQ_API_KEY") else "âŒ"
    await msg.answer(f"ðŸ©º *API & System Health*\nâ”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n\nDatabase: {db_ok}\nRedis: {redis_ok}\nAI (Groq): {ai_ok}\nBot online: âœ…", parse_mode=None)

@dp.message(Command("setreminder"))
async def cmd_setreminder(msg: Message):
    if msg.from_user.id not in ADMIN_IDS: return await msg.answer(msg_error_admin_only())
    parts = msg.text.split()
    if len(parts) < 2: return await msg.answer("×©×™×ž×•×©: /setreminder <HH:MM> UTC\n×“×•×’×ž×”: /setreminder 14:00")
    time_str = parts[1]
    try:
        ...
        conn.commit()
        conn.close()
        await msg.answer(f"âœ… ×ª×–×›×•×¨×ª ×™×•×ž×™×ª × ×§×‘×¢×” ×œ×©×¢×” {time_str} UTC. ××©×œ×— ×“×•×— ×›×œ ×™×•×.")
    except:
        await msg.answer("âŒ ×¤×•×¨×ž×˜ ×œ× ×ª×§×™×Ÿ. ×”×©×ª×ž×© HH:MM (24h).")
...
async def send_daily_reminder():
    ...
    conn.close()
    text = f"ðŸŒ… *Daily SLH Report*  {datetime.date.today()}\n\nâœ… Check-ins today: {checked}/{total}\nðŸ’° Revenue (24h): ${revenue:.2f}\n\nðŸ‘‰ /start to engage!"
    await bot.send_message(ADMIN_IDS[0], text, parse_mode=None)

# ========================== AI ==========================
@dp.message(F.text & ~F.text.startswith("/"))
async def ai_chat(msg: Message):
    groq_key = os.getenv("GROQ_API_KEY")
    if not groq_key: return await msg.answer("AI ×œ× ×ž×•×’×“×¨")
    try:
        resp = requests.post(
...
def build_doctor_report(railway, db, redis, ai):
    def icon(r): return "âœ…" if r.get("ok") is True else ("âšª" if r.get("ok") is None else "âŒ")
    lines = ["ðŸ©º *SLH System Doctor*", "â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”", ""]
    lines.append(f"{icon(railway)} *Railway* â€” {railway.get('status','?')}")
    lines.append("")
    if db.get("ok"): lines.append(f"{icon(db)} *Database* â€” {db.get('latency_ms','?')}ms")
    else: lines.append(f"{icon(db)} *Database* â€” {db.get('detail','error')}")
    if redis.get("ok") is None: lines.append(f"{icon(redis)} *Redis* â€” not configured")
    elif redis.get("ok"): lines.append(f"{icon(redis)} *Redis* â€” {redis.get('latency_ms','?')}ms")
    else: lines.append(f"{icon(redis)} *Redis* â€” error")
    lines.append(f"{icon(ai)} *{ai.get('provider','AI')}* â€” {ai.get('detail','key present' if ai.get('ok') else 'missing')}")
    lines.append("")
    lines.append("â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”")
    if all(c.get("ok") is True for c in [railway, db, ai]): lines.append("ðŸŸ¢ *All systems operational*")
    elif any(c.get("ok") is False for c in [railway, db, ai]): lines.append("ðŸ”´ *Issues detected*")
    else: lines.append("ðŸŸ¡ *Partial checks*")
    return "\n".join(lines)
