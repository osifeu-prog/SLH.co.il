"""
SLH AI Spark ׳’ג‚¬ג€ Subscription DB layer.
...
# Phase B mirror ׳’ג‚¬ג€ bot pushes state to slh-api Postgres so the Mini App
...
async def init_db() -> None:
    ...
    if not os.path.exists(MIGRATION_FILE):
        # Embedded fallback ׳’ג‚¬ג€ keeps the bot bootable even if file is missing
...
async def _mirror_to_pg(sub: SubscriptionRow) -> None:
    ...
    Never raises ׳’ג‚¬ג€ SQLite stays canonical for quota.check (latency-sensitive).
...
async def mrr_ils() -> float:
    """Monthly recurring revenue: sum of active paid subscriptions ׳ֲ¿ֲ½- tier price."""
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
