...
async def cmd_game(msg: types.Message):
    ...
    await msg.answer("🎲 Guess the Number! (1-20)\nSend /guess <number>\nCorrect = 50 points")

@dp.message(Command("guess"))
async def cmd_guess(msg: types.Message):
    parts = msg.text.split()
    if len(parts) < 2:
    ...
    if uid not in user_guesses:
        await msg.answer("Start with /game first")
        return
    secret = user_guesses.pop(uid)
    if guess == secret:
        async with pool.acquire() as conn:
            await conn.execute("UPDATE users SET points = points + 50 WHERE telegram_id = $1", uid)
