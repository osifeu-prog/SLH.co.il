...
async def api_risk_entities():
    async with get_pg_conn() as conn:
        rows = await conn.fetch("""
            SELECT entity_id, canonical_name, score_zuz, approved_reports, verified_evidence, eligible_for_publication
...
async def api_risk_external_watch():
    async with get_pg_conn() as conn:
        rows = await conn.fetch("""
            SELECT
                w.asset_key, w.asset_name, w.chain, w.contract_address,
