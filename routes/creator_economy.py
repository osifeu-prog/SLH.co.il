...
XP formula:
  ...
  where:
    total_portfolio_value_now = sum(SLH_held ï¿½ current_SLH_price
                                   + AIC_held ï¿½ current_AIC_price
...
A user with 0 fiat invested is "xp = 8 (pure gain)" - not counted in index.
"""
...
async def _compute_xp(conn, user_id: int) -> dict:
    ...
    return {
        ...
        "xp_ratio": xp_ratio,
        "xp_display": "8" if xp_ratio is None else f"{xp_ratio:.2f}",
        "status": (
...
async def slh_index():
    ...
    async with _pool.acquire() as conn:
        ...
        return {
            ...
            "trend": trend,
            "peak_xp_label": "8" if any(r.get("xp_ratio") is None for r in rows) else round(xps[-1], 2),
...
async def purchase_complete(req: PurchaseCompleteReq):
    ...
    return {
        ...
        "message": f"????? ?????? ï¿½ ????? #{seller_id} ???? ${usd:.2f}, XP ?????.",
    }
